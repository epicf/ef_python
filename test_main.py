...
def test_main(mocker, capsys, tmpdir):
    ...
    main()
    expected_before_warnings = f"""Config file is:  {config}
[ TimeGrid ]
total_time = 10
time_save_step = 5
time_step_size = 1
...
"""
    expected_after_warnings = """Writing step 0 to file out_0000000.h5
...
"""
    ...
    assert out.startswith(expected_before_warnings)
    assert out.endswith(expected_after_warnings)
    warnings = out[len(expected_before_warnings):len(out)-len(expected_after_warnings)]
    warning = "warning: scipy.sparse.linalg.cg info:  1000\n"
    assert warnings in ("", warning, warning+warning)
